    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          // transform points to car's reference frame (x,y,phi) == (0,0,0)
          ...
          // initialize vectors from points with some wicked shorthand
          Eigen::Map<Eigen::VectorXd> vecX(&tX[0], 6);
          Eigen::Map<Eigen::VectorXd> vecY(&tY[0], 6);
          ...
          double cte = polyeval(coeffs,0); // y is 0
          ...
          */
          ...
          {
            (k % 2 == 0) ?   mpc_x_vals.push_back(values[k]) :   mpc_y_vals.push_back(values[k]);
          }
        ...
        // Manual driving
        std::string msg = "42[\"manu 0.0001*al\",{}]";
        ws.send(msg.data(), msg.length(), uWS::OpCode::TEXT);
      }
    }
  });

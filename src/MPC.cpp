...
size_t N = 10;
double dt = 0.1;
double ref_v = 70; // 5 m/s reference velocity
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars){
    ...
    // make it expensive to change between actuations over time
    for (int i = 0; i < N - 2; i++) {
    ...
    for (int t = 1; t < N; t++) {
      ...
      // Evaluate polynomial for CTE using all coefficients
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // set non-actuators to entire range of values
